...
hh_path <- "../data/CMHR71DT/CMHR71FL.DTA"
ind_path <- "../data/CMIR71DT/CMIR71FL.DTA"
#########
...
out <- clusNode(subset_data, 4, ncores = nCores)
...
# Ordered Logisitic Regressions Analysis
...
grouped <- clus_config_ordered(data = dhs, i=1, tab=tab1, nCores = nCores)
...
##### Get temporary ranks for the clusters
#grouped_meta$tmp_ranks <- rank(desc(grouped_meta$ave_asset))

##### NEED TO EDIT THIS PART BASED ON DECISION FROM ALAN
##### Check if there are any ties, and break them if so
##### Set up as not equal to 5 now, since we always use this
##### As the number of clusters
# if(unique(tmp_ranks) != 5){
# }

##### NEED TO EDIT THIS PART BASED ON DECISION FROM ALAN

### rank 5: highest asset, rank 1: lowest asset
grouped_meta$rank <- rank(grouped_meta$ave_asset)
score <- sum(table(grouped_meta$rank)-1)
...
# Merge the datasets
merged_data$clus <- paste0(unname(unlist(merged_data[,c(tab[i,2])])), 
                      unname(unlist(merged_data[,c(tab[i,3])])),
                      unname(unlist(merged_data[,c(tab[i,4])])),
                      unname(unlist(merged_data[,c(tab[i,5])]))
  )

ord_log_data <- merge(merged_data,group_final[,c("clus","rank")],by="clus")
ord_log_data$rank <- factor(ord_log_data$rank)
```

```{r}
##### Proportion of children deceased
ord_log_decat <- MASS::polr(de_cat ~ rank, data = ord_log_data, weights = wt)
summary(ord_log_decat)
```

```{r}
##### Proportion of children deceased
ord_log_educ <- MASS::polr(factor(v149) ~ rank, data = ord_log_data, weights = wt)
summary(ord_log_educ)
```

```{r}
#### Primary source of healthcare
ord_log_health <- MASS::polr(factor(health) ~ rank, data = ord_log_data, weights = wt)
summary(ord_log_health)
```
