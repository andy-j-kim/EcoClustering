---
title: "Economic Clustering Summary Report: Mali 2018"
output: 
  word_document:
...
hh_path <- "~/Box/Project 1/DHS Data/MALI_2018/MLHR7ADT/MLHR7AFL.DTA"
ind_path <- "~/Box/Project 1/DHS Data/MALI_2018/MLIR7ADT/MLIR7AFL.DTA"
```
...
merged_dhs <- hh %>% 
  ...
  merge(ind_subset[!duplicated(ind_subset), ], # something off about Mali's individual codes
        by.x=c("hv001", "hv002"), 
...
if("edutab" %in% names(val_tables)){
  edutab.f <- val_tables$edutab %>%
    ...
    mutate(`W. Avg.` = round(c(val_tables$ordered_eduid$V2, 
                         sum(prop.table(as.matrix(colSums(val_tables$edutab[,-1]),1)) * (val_tables$edutab %>%
                                                                                           names() %>% 
                                                                                           `[`(-1) %>%
                                                                                           as.numeric()))),2)) %>%
    flextable::flextable() %>% 
