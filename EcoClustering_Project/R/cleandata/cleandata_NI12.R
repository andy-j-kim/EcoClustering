...
dataset <- haven::read_dta("~/Box/Project 1/DHS Data/NIGER 2012/NIHR61DT/NIHR61FL.DTA")
...
dataclean <- dataset %>% 
  ...
  #7
  dplyr::select(-c(hv237c, hv246c, hv246g, hv246h, sh106b, sh141)) %>% 

  # move weights to front of the dataset
  dplyr::select(wt, everything()) %>% 
  
  # Filtering 9s out after
