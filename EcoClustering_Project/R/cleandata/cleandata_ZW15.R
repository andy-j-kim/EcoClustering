...
dataset <- haven::read_dta("~/Box/Project 1/DHS Data/ZIMBABWE_2015/ZWHR72DT/ZWHR72FL.DTA")
...
dataclean <- dataset %>% 
  ...
  dplyr::filter(hv201a !=8, hv237 != 8, hv253 != 8) %>% 
  #6
  dplyr::select(-which(purrr::map_lgl(., detect_imbalance, max_imbalance)), wt) %>% 
  
  #7
  dplyr::select(-c(hv232, hv232y, hv246c, hv246e, hv246g, hv252, hv253a)) %>% 
